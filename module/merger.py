...
def _prune_weak_siblings(existing_entries, new_entry):
    for entry in existing_entries:
        if entry.source != new_entry.source:
            continue
...
def merge_all(parser_results_list: list[ParsedDataDTO]):
    ...
    for scan_file_result in parser_results_list:
        ...
        for ip, host_info in scan_data.items():
            if ip not in merged_db:
                merged_db[ip] = HostDTO(ip=ip)

            entry = merged_db[ip]

            if command:
                entry.discovery_commands.add(command)

            _merge_traked_field(entry.hostnames, host_info.hostnames, command)
            #TODO: finish the implementation of OsDTO
            # _merge_traked_field(entry.os_info, host_info.os, tool_name)
            _merge_traked_field(entry.cpes, host_info.cpes, command)
    ...
    final_output = FinalReportDTO(total_hosts=len(merged_db), commands=list(command_set), hosts={})

    sorted_ips = sorted(merged_db.keys(), key=lambda ip: ipaddress.ip_address(ip))
    for ip in sorted_ips:
        host_obj = merged_db[ip]
        # host_obj.hostnames = list(host_obj.hostnames)
        host_obj.hostnames = dict(host_obj.hostnames)
